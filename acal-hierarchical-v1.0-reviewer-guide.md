...
[HRP-ACAL] Section 7 gives one example per scheme, and — like [MDP-ACAL] Section 8 — deliberately spans all three ACAL representations rather than staying in XML throughout, since two of the three node-identity schemes have no representation dependency at all ([Section 5](acal-hierarchical-v1.0.md#5-node-identity-schemes)'s dependency table). [Section 7.1](acal-hierarchical-v1.0.md#71-nodes-in-xml-documents) (XML documents) is XACML v4.0/XML, necessarily, since that scheme only applies to XML `Content`. [Section 7.2](acal-hierarchical-v1.0.md#72-nodes-identified-by-uri) (URI) is JACAL/JSON. The third, [Section 7.3](acal-hierarchical-v1.0.md#73-nodes-identified-by-ancestor-attributes), is YACAL/YAML, and is the one worth reading closely if you want to see the "no narrowing" claim in concrete form: a `resource-id` of `"Project Falcon"` — deliberately an opaque `string` name, not a hierarchy-encoding URI, so the example can't be mistaken for one the [Section 5.2](acal-hierarchical-v1.0.md#52-nodes-identified-by-uri) URI scheme could express on its own — whose immediate parent (`resource-parent`) is *separately* represented twice: once as a `string` display name (`"Platform Engineering Team"`) and once as an `anyURI` alias (`acme:org/platform-team`), both sharing the `AttributeId` `urn:oasis:names:tc:acal:1.0:resource:resource-parent`. Under the pre-#120 constraint this would have been an illegal duplicate. Under the current constraint it is two distinct, valid objects, because they differ in `DataType`. A policy that only understands the `anyURI` representation can simply ignore the `string` one.
...
- **Verify the JSON and YAML worked examples independently, not just the XML one.** Sections 7.2 and 7.3 are the profile's evidence that the URI and ancestor-attribute schemes are genuinely representation-agnostic, not XML with a format-agnostic claim attached. Parse each one and, ideally, schema-validate the resulting data structure against `acal-core-json-v1.0-schema.json` — the same schema applies to parsed YAML as to JSON, since YACAL mirrors JACAL's property names exactly, but the YAML text itself must be parsed into that structure first, not fed to a JSON validator as text. Don't just read the examples as prose illustrations.
