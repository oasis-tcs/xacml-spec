...
| `CombinerInput` | exactly one `PolicyReference`, to this role's PP | the role's `Rule`/nested `Policy` permissions; `PolicyReference` to each *immediately* junior role's PP; optionally a HasPrivilegesOfRole `Rule` |
