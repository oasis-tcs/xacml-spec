...
**HasPrivilegesOfRole cardinality is now constrained.** [RBAC-ACAL] §5.3.4 constraint 1 requires the request's `action-id` to carry exactly the reserved value (a request mixing it with an ordinary action gets no defined HasPrivilegesOfRole answer), and reads a resource role attribute naming several roles existentially — `Permit` if the subject has the privileges of any one of them. **What to check:** that a worked example or test carrying a multi-valued resource role attribute, if one is added, is read this way, not as an error.
