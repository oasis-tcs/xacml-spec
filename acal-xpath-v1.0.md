...
class NamespaceDeclarationsType <<dataType>> {
  {field} + NamespaceDeclaration: NamespaceDeclarationType [*] {unordered, unique} {{OCL} self->isUnique(Prefix)}
}
...
A `NamespaceDeclarationsType` object contains the following property:

`NamespaceDeclaration` [Any Number]

: The namespace-prefix bindings, each a `NamespaceDeclarationType` object. No two SHALL have the same `Prefix`. An object with no bindings denotes an expression that needs none.
...
class XPathAttributeSelectorType <<dataType>> extends AttributeSelectorType {
  ...
  + Namespaces: NamespaceDeclarationsType [1]
}
@enduml
```
...
: The `NamespaceDeclarationsType` object (see [Section 5.2](#52-namespacedeclarationstype-optional)) holding the namespace-prefix bindings needed to resolve the namespace prefixes used within the `Path` expression. In a non-XML representation this property is REQUIRED — an expression that needs no namespace bindings still carries an empty `Namespaces` — so that a selector's namespace resolution never depends on anything other than the selector itself. A producer SHALL include at least every namespace binding the `Path` expression references, and SHOULD include the expression's complete namespace context — for a selector translated from an XML source, the in-scope namespaces of the source element; for one authored directly in a non-XML representation, every binding the expression is meant to resolve against. Where a producer cannot reliably determine which bindings the expression references, it SHALL include that complete context. This profile defines no normative minimization over the lexical content of an XPath expression string (unlike the "visibly utilized" [[exc-c14n](#exc-c14n)] test it uses for XML infoset content elsewhere). In an XML representation the prefixes resolve from ordinary XML in-scope namespaces instead (see the bolded note below).
