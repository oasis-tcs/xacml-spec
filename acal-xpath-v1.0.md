...
The namespace context a `Namespace` property must carry (see [Section 5.3.3](#533-attributeselectortype-extension---xpathattributeselectortype), [Section 5.3.4](#534-entityattributeselectortype-extension---xpathentityattributeselectortype), [Section 5.3.5](#535-datatype-extension---xpathexpressionvaluetype)) excludes bindings XPath and XML define implicitly, such as the reserved `xml` prefix (always bound to `http://www.w3.org/XML/1998/namespace`); implementations MUST treat this binding as available without it appearing in `Namespace`, in every representation.
...
: The namespace prefix bindings (zero or more `NamespaceDeclarationType` objects, see [Section 5.2](#52-namespacedeclarationtype-optional)) needed to resolve the namespace prefixes used within the `Path` expression. In a non-XML representation this property is REQUIRED — an expression that needs no namespace bindings still carries an empty `Namespace` — so that a selector's namespace resolution never depends on anything other than the selector itself. A producer SHALL include at least every namespace binding the `Path` expression references, and SHOULD include the expression's complete namespace context — for a selector translated from an XML source, the in-scope namespaces of the source element; for one authored directly in a non-XML representation, every binding the expression is meant to resolve against. Where a producer cannot reliably determine which bindings the expression references, it SHALL include that complete context. This profile defines no normative minimization over the lexical content of an XPath expression string (unlike the "visibly utilized" [[exc-c14n](#exc-c14n)] test it uses for XML infoset content elsewhere). In an XML representation the prefixes resolve from ordinary XML in-scope namespaces instead (see the bolded note below). There SHALL NOT be more than one `NamespaceDeclarationType` object for the same `Prefix`.
...
The `XPathEntityAttributeSelectorType` object type extends the `EntityAttributeSelectorType` object type with the same `ContextSelectorId` and `Namespace` properties as `XPathAttributeSelectorType` ([Section 5.3.3](#533-attributeselectortype-extension---xpathattributeselectortype)).
...
: When an implementation serializes such a value into a `Request` or `Policy`, it MAY omit the local `XPathVersion` only if that container carries an applicable Defaults object whose `XPathVersion` **denotes the same XPath version** as the value's effective version — the comparison is between the two `IdentifierType` values after any `{ShortId}` expansion, and a deprecated version identifier is not the same as its replacement unless [Section 6](#6-xpath-definitions) states an equivalence. Otherwise it MUST emit the local `XPathVersion`. A producer MUST emit a local `XPathVersion` for a value it serializes in a decision response (`Notice`, `AttributeAssignment`), which carries no Defaults object and may be logged, forwarded, or processed apart from the request that produced it. How an implementation holds the value internally (a parsed object, a compiled expression, a cache entry) is not constrained, provided it preserves the value's effective version and namespace context; the rules here apply when it emits an ACAL document or interchange value.

`Namespace` [Required]

: The namespace prefix bindings (zero or more `NamespaceDeclarationType` objects, see [Section 5.2](#52-namespacedeclarationtype-optional)) needed to resolve the namespace prefixes used within the `XPath` expression, realized by representation exactly as for the selector types' `Namespace` property ([Section 5.3.3](#533-attributeselectortype-extension---xpathattributeselectortype)). In a non-XML representation the value carries its own `Namespace` mapping; it is REQUIRED — an expression that needs no namespace bindings still carries an empty `Namespace` (`{}`), distinct from having forgotten to state the namespace context — so that the value's namespace resolution never depends on anything other than the value itself. Unlike `XPathVersion`, `Namespace` has no Defaults-object fallback: a single shared namespace table cannot faithfully represent every combination of XML-sourced expressions (the defect issue #134 corrected), so a non-XML value always states its context locally, `{}` included. A producer SHALL include at least every binding the `XPath` expression references, and SHOULD include the expression's complete namespace context — for a value translated from an XML source, the in-scope namespaces of the source element; for one authored directly in a non-XML representation, every binding the expression is meant to resolve against. Where a producer cannot reliably determine which bindings are referenced, it SHALL include that complete context (as with the selector `Namespace` property in [Section 5.3.3](#533-attributeselectortype-extension---xpathattributeselectortype), this profile defines no normative minimization over the lexical content of an XPath string). In an XML representation the prefixes resolve from the ordinary XML [in-scope namespaces] [[INFOSET](#infoset)] of the element carrying the value, which may be established by a declaration on an ancestor element — the same mechanism [[XACML 3.0](#xacml)] has always used, and the same one this profile applies to a selector's `Path`; this profile does not require the `xmlns:*` declarations to sit on the carrying element itself. Because a `xpathExpression` value — unlike a selector's `Path` — can be produced in one document and emitted into another (a response `Notice` or `AttributeAssignment`, another policy), an implementation emitting it into an ACAL document or interchange value MUST resolve the expression's complete effective in-scope namespace context at the source and re-express it so the expression stays resolvable at the destination: as the `Namespace` mapping when the destination is a non-XML representation, or as in-scope `xmlns:*` declarations (on the carrying element or an ancestor of it in the new document) when the destination is XML. This is the ordinary obligation that accompanies moving any namespace-qualified XML content between documents; it is called out here because a prefix used only inside the `XPath` string is character data, not markup, so XML well-formedness alone does not enforce it. There SHALL NOT be more than one `NamespaceDeclarationType` object for the same `Prefix`.
