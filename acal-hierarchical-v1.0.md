...
- **Function argument order.** XACML 3.0 HRP §4.3's `regexp-uri-match` takes the URI argument first and the regular expression second. ACAL's equivalent, `urn:oasis:names:tc:acal:1.0:function:anyURI-regexp-match`, takes the regular expression first, matching the argument order of ACAL's other `*-regexp-match` functions. Any policy text carried over from an XACML 3.0 HRP example MUST have its arguments swapped — see [Section 6.3](#63-matching-uris).
