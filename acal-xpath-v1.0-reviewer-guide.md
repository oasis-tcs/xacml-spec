...
### The Defaults types lost their `Namespace`; `xpathExpression` values gained an optional local `XPathVersion` (issue #134)
...
- **Namespace bindings move to the expression.** Both Defaults types lose their `Namespace` property. In JACAL/YACAL each `xpathExpression` value ([Section 5.3.5](acal-xpath-v1.0.md#535-datatype-extension---xpathexpressionvaluetype)) and each selector carries its own `Namespace` mapping. XML is *unchanged* — prefixes resolve from ordinary in-scope namespaces, ancestor declarations included, exactly as in XACML 3.0 (XML's nested scoping never had the defect). The only new XML obligation is at the boundary: an implementation translating a value into JACAL/YACAL, or materializing one into a new document such as a response `Notice`, MUST carry the expression's complete effective namespace context across.
...
**What to check:** a reference anywhere in this repository to a `Namespace` property on `XPathRequestDefaultsType`/`XPathPolicyDefaultsType`, or to `XPathVersion` being *required* on an `xpathExpression` value, is stale content from before this change. `XPathRequestDefaultsType` itself is **not** gone — an earlier iteration of this branch removed it and that framing may survive in stale notes.

### `NamespaceDeclarationType` is a mapping in non-XML representations, and appears in more places now

`NamespaceDeclarationType` ([Section 5.2](acal-xpath-v1.0.md#52-namespacedeclarationtype-optional)) is still **not used in XACML v4.0/XML**, which has native namespace support. In XML, both a selector's `Path` and an `xpathExpression` value's `XPath` resolve prefixes the same way: ordinary XML in-scope-namespace lookup, which can be satisfied by an ancestor declaration (every canonical XML Rule example binds `md` on the outer `<Policy>`, not on each element that uses it) — issue #134's defect was specific to a single flat namespace table in the non-XML representations, and XML's nested scoping never had it. In JACAL/YACAL, `xpathExpression` values and both selector types carry a `Namespace` property: a mapping from prefix to namespace name (`{"md": "urn:example:..."}` in JSON, an equivalent mapping in YAML) rather than an array of `{Prefix, Name}` pair-objects — a shape change from before issue #134, made because the old array shape was reused so much more often once every value and selector needed its own copy that the array-of-objects form became noticeably more awkward than a plain mapping, in both formats. **What to check:** if you see `NamespaceDeclarationType`/`Namespace` used as an array of pair-objects anywhere in this repository, or a `Namespace` property declared inside an XACML/XML example (it never is — that's a JACAL/YACAL-only property), that's stale content from before issue #134. Do not flag an XML value or selector that relies on an ancestor `xmlns:*` as a defect — that is the correct, unchanged behavior. Do flag an XML `xpathExpression` value materialized into a response `Notice` whose XPath prefixes are not in scope anywhere in that response document.
